...
from Methods.FasterGSDash.Loss import FasterGSLoss
from Methods.FasterGSDash.utils import enable_expandable_segments, carve
...
@Framework.Configurable.configure(
    ...
    DENSIFICATION_END_ITERATION=27_000,  # extended from FasterGS default (14900) to match DashGaussian
    DENSIFICATION_INTERVAL=100,
    DENSIFICATION_GRAD_THRESHOLD=0.0002,  # scaled by render_scale² inside dash_density_control_topk
...
class FasterGSTrainer(GuiTrainer):
    """FasterGS trainer extended with DashGaussian resolution + primitive scheduling."""
    ...
    def increase_sh_degree(self, *_) -> None:
        """Increase SH degree, but only once the resolution is near-full (decision D2)."""
    ...
    def densify(self, iteration: int, dataset: 'BaseDataset') -> None:
        ...
        if self.USE_MCMC:
            self.model.gaussians.mcmc_densification(min_opacity=0.005, cap_max=getattr(self, 'MAX_PRIMITIVES', 1_000_000))
        else:
    ...
    def training_iteration(self, iteration: int, dataset: 'BaseDataset') -> None:
        """Training step with DashGaussian resolution scaling applied to GT and renderer."""
        self.model.train()
        dataset.train()
        self.loss.train()
        ...
        # downsample GT with anti-aliased area interpolation (decision A1)
        rgb_gt = view.rgb
