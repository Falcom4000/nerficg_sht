...
namespace faster_gs::rasterization::kernels {
        ...
        const uint total_sh_bases_rest)
    {
        ...
        grad_accum_sh_coefficients_0[primitive_idx] += dL_dsh_coefficients_0;
        ...
        if (active_sh_bases > 1) {
            ...
            grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 0] += dL_dsh_coefficients_rest_0;
            const float3 dL_dsh_coefficients_rest_1 = C1 * z * grad_color;
            grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 1] += dL_dsh_coefficients_rest_1;
            const float3 dL_dsh_coefficients_rest_2 = -C1 * x * grad_color;
            grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 2] += dL_dsh_coefficients_rest_2;
            ...
            if (active_sh_bases > 4) {
                ...
                grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 3] += dL_dsh_coefficients_rest_3;
                const float3 dL_dsh_coefficients_rest_4 = -C2a * yz * grad_color;
                grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 4] += dL_dsh_coefficients_rest_4;
                const float3 dL_dsh_coefficients_rest_5 = (C2b * zz - C2c) * grad_color;
                grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 5] += dL_dsh_coefficients_rest_5;
                const float3 dL_dsh_coefficients_rest_6 = -C2a * xz * grad_color;
                grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 6] += dL_dsh_coefficients_rest_6;
                const float3 dL_dsh_coefficients_rest_7 = C2d * (xx - yy) * grad_color;
                grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 7] += dL_dsh_coefficients_rest_7;
                ...
                if (active_sh_bases > 9) {
                    ...
                    grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 8] += dL_dsh_coefficients_rest_8;
                    const float3 dL_dsh_coefficients_rest_9 = C3c * xy * z * grad_color;
                    grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 9] += dL_dsh_coefficients_rest_9;
                    const float3 dL_dsh_coefficients_rest_10 = y * (C3d - C3e * zz) * grad_color;
                    grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 10] += dL_dsh_coefficients_rest_10;
                    const float3 dL_dsh_coefficients_rest_11 = z * (C3f * zz - C3g) * grad_color;
                    grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 11] += dL_dsh_coefficients_rest_11;
                    const float3 dL_dsh_coefficients_rest_12 = x * (C3d - C3e * zz) * grad_color;
                    grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 12] += dL_dsh_coefficients_rest_12;
                    const float3 dL_dsh_coefficients_rest_13 = C3h * z * (xx - yy) * grad_color;
                    grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 13] += dL_dsh_coefficients_rest_13;
                    const float3 dL_dsh_coefficients_rest_14 = x * (C3b * yy - C3a * xx) * grad_color;
                    grad_accum_sh_coefficients_rest[primitive_idx * total_sh_bases_rest + 14] += dL_dsh_coefficients_rest_14;
