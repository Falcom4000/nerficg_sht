...
void faster_gs::rasterization::score_forward(
    std::function<char* (size_t)> resize_primitive_buffers,
    std::function<char* (size_t)> resize_tile_buffers,
    std::function<char* (size_t)> resize_instance_buffers,
    const float3* means,
    ...
    const float compact_box_mult)
{
    ...
    cudaMemset(primitive_buffers.n_visible_primitives, 0, sizeof(uint));
    cudaMemset(primitive_buffers.n_instances, 0, sizeof(uint));
    cudaMemset(accum_metric_counts, 0, sizeof(int) * n_primitives);
    ...
    CHECK_CUDA(config::debug, "cub::DeviceRadixSort::SortPairs (depth, score)")
    ...
    CHECK_CUDA(config::debug, "apply_depth_ordering (score)")
    ...
    CHECK_CUDA(config::debug, "cub::DeviceScan::ExclusiveSum (primitive_buffers.offset, score)")
    ...
    CHECK_CUDA(config::debug, "cub::DeviceRadixSort::SortPairs (tile, score)")
    ...
    if (n_instances > 0) {
        ...
        CHECK_CUDA(config::debug, "extract_instance_ranges (score)")

        kernels::forward::blend_score_cu<<<grid, block>>>(
            tile_buffers.instance_ranges,
            instance_buffers.primitive_indices.Current(),
            primitive_buffers.mean2d,
            primitive_buffers.conic_opacity,
        ...
        CHECK_CUDA(config::debug, "blend_score")
    }
}
