    ...
    const bool apply_invisible_momentum)
{
    ...
    if (apply_invisible_momentum) {
        // For Gaussians that touched zero tiles: apply a momentum-based Adam step
        // (no gradient, but accumulated momentum still nudges parameters).
        // This is disabled when render_scale > 1 (FasterGSFusedDash low-res phases)
        // because many Gaussians that ARE visible at full resolution have zero
        // touched-tiles at the reduced resolution — letting momentum move them
        // without gradients causes quality-degrading drift.
