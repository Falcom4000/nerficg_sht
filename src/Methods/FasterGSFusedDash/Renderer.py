...
def extract_settings(
    ...
    if not isinstance(view.camera, PerspectiveCamera):
        raise Framework.RendererError('FasterGSFusedDash renderer only supports perspective cameras')
...
class FasterGSFusedDashRenderer(BaseRenderer):
    """FasterGSFused renderer extended with DashGaussian render_scale support."""

    def __init__(self, model: 'BaseModel') -> None:
        super().__init__(model, [FasterGSFusedDashModel])
    ...
    def render_image_training(self, view: View, update_densification_info: bool, bg_color: torch.Tensor, adam_step_count: int, autograd_dummy: torch.Tensor, render_scale: int = 1) -> tuple[torch.Tensor, torch.Tensor]:
