...
@Framework.Configurable.configure(
    ...
    DENSIFICATION_START_ITERATION=500,  # match DashGaussian (was 600)
...
class FasterGSFusedDashTrainer(GuiTrainer):
    ...
    def setup_gaussians(self, _, dataset: 'BaseDataset') -> None:
        ...
        # pre-cache downsampled GT images for low-resolution training phases
        self._gt_cache = {}
        if not self.USE_RANDOM_BACKGROUND_COLOR:
            ...
            Logger.log_info(f'pre-cached GT images for {len(self._gt_cache)} (view, scale) pairs')
    ...
    def increase_sh_degree(self, *_) -> None:
        """Increase SH degree unconditionally every 1000 iters (matches DashGaussian)."""
    ...
    def training_iteration(self, iteration: int, dataset: 'BaseDataset') -> None:
        ...
        # GT image: use pre-cached version when available, else compute on the fly
