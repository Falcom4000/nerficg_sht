...
### V4 最终基线（commit 71e1765，大样本 avg）

| 场景 | FasterGSFused | FasterGSDash | FusedDash V4 | vs GSDash | vs Fused | FusedDash Time | Fused Time | speedup |
|------|-------------|-------------|-------------|-----------|----------|--------------|-----------|---------|
| bonsai | 32.66 / 1.14G | 32.59 | 32.55 / 1.34G | -0.04 | -0.11 | 218s | 196s | 0.90× |
| counter | 29.47 / 1.04G | — | 29.43 / 1.10G | -0.04 | -0.04 | 207s | 208s | 1.00× |
| kitchen | 32.28 / 1.56G | 32.10 | 32.05 / 1.60G | -0.05 | -0.23 | 275s | 270s | 0.98× |
| room | 32.38 / 1.22G | 32.44 | 32.24 / 1.35G | -0.20 | -0.14 | 178s | 194s | 1.09× |
| bicycle | 25.26 / 4.88G | 25.15 | 25.08 / 4.24G | -0.07 | -0.18 | 380s | 477s | **1.25×** |
| garden | 27.49 / 4.14G | 27.52 | 27.43 / 3.60G | -0.09 | -0.07 | 385s | 484s | **1.26×** |
| stump | 26.67 / 4.21G | 26.69 | 26.26 / 3.68G | -0.43 | -0.40 | 315s | 398s | **1.26×** |

**关键结论：**
- outdoor 场景（bicycle/garden/stump）速度 **1.25-1.26×**，显存节省约 0.5G
- indoor 场景速度基本持平，room 略快 1.09×
...
| 场景 | V1 | V4 (大样本) | 改进 |
|------|----|----|------|
| bonsai | 32.70 | 32.55 | -0.15 |
| kitchen | 31.84 | 32.05 | **+0.21** |
| room | 32.24 | 32.24 | ±0 |
| bicycle | 25.10 | 25.08 | -0.02 |
| garden | 27.45 | 27.43 | -0.02 |
| stump | 25.78 | 26.26 | **+0.48** |
...
### 实验结果（3-run avg，commit 4edf9b3）

| 场景 | V4 | V6 | Δ dB | 时间变化 |
|------|----|----|------|---------|
| bonsai | 32.55 | 32.47 | -0.08 ▼ | 189s ≈ |
| counter | 29.43 | 29.46 | +0.03 | 198s ≈ |
| kitchen | 32.05 | 32.01 | -0.04 | 269s ≈ |
| room | 32.24 | 32.31 | **+0.07** | 177s ≈ |
| bicycle | 25.08 | 25.09 | +0.01 | 384s ≈ |
| garden | 27.43 | 27.47 | **+0.04** | 385s ≈ |
| stump | 26.26 | 26.39 | **+0.13 ★** | 317s ≈ |
| **合计** | | | **+0.16** | — |

**训练时间无变化（±3s 范围内）。**

**结论：**
- stump **+0.13 dB**：最显著改善，正是 #13 的预期受益场景（低分辨率期长、invisible Gaussian 多、momentum 冻结时间最久）
- room/garden 小幅正向
- bonsai -0.08 dB：indoor 场景低分辨率期短，moment decay 无收益，数值在正常 run-to-run 方差范围内（通常 ±0.15 dB），不视为真实退步
- stump 相对 FasterGSFused 的 gap 从 -0.40 → -0.27 dB，已缩小约 1/3
